...
// Desplácese por el carrusel en la posición adecuada 
carousel.classList.add("no-transition");
carousel.scrollLeft = carousel.offsetWidth;
carousel.classList.remove("no-transition");
// Agregue detectores de eventos para los botones de flecha para desplazar el carrusel
...
const infiniteScroll = () => {
    ...
    // Se borra el tiempo de espera si el mouse no está sobre el carrusel
    clearTimeout(timeoutId);
    if(!wrapper.matches(":hover")) autoPlay();
}
const autoPlay = () => {
    if(window.innerWidth < 400 || !isAutoPlay) return; // Volver si la ventana es menor que 400 o isAutoPlay es falso
    // Reproduce automáticamente el carrusel después de cada 4000 ms
